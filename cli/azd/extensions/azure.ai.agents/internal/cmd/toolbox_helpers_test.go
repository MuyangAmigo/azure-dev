...
func TestValidateProjectEndpoint(t *testing.T) {
	cases := []struct {
		name    string
		input   string
		wantErr string // exterrors code; "" = success
	}{
		{
			name:  "happy https",
		...
		},
		{
			name:    "empty",
			input:   "  ",
			wantErr: exterrors.CodeInvalidProjectEndpoint,
		},
		{
			name:    "missing scheme",
			input:   "acct.services.ai.azure.com/api/projects/proj",
			wantErr: exterrors.CodeInvalidProjectEndpoint,
		},
		{
			name:    "http rejected",
			input:   "http://acct.services.ai.azure.com/api/projects/proj",
			wantErr: exterrors.CodeInvalidProjectEndpoint,
		},
		{
			name:    "no host",
			input:   "https:///api/projects/proj",
			wantErr: exterrors.CodeInvalidProjectEndpoint,
		},
	}
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got, err := validateProjectEndpoint(tc.input, endpointSourceFlag)
			if tc.wantErr != "" {
				le := requireLocalError(t, err, tc.wantErr)
				assert.Contains(t, le.Message, "--project-endpoint")
				return
			}
			require.NoError(t, err)
			assert.NotEmpty(t, got)
			assert.False(t, hasTrailingSlash(got))
		})
	}
}
